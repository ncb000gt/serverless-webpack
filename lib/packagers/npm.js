...
function npmCommand(options) {
  if (!_.isNil(options) && options.cmd !== null && options.cmd !== undefined) {
    return options.cmd;
  }
...
class NPM {
  ...
  static getProdDependencies(cwd, depth, options) {
    // Get first level dependency graph
    const command = npmCommand(!_.isNil(options) ? options.prodDependencies || options : options);
    const args = [
      'ls',
  ...
  static install(cwd, options) {
    const command = npmCommand(!_.isNil(options) ? options.install || options : options);
    const args = ['install'];

    return Utils.spawnProcess(command, args, { cwd }).return();
  }

  static prune(cwd, options) {
    const command = npmCommand(!_.isNil(options) ? options.prune || options : options);
    const args = ['prune'];

    return Utils.spawnProcess(command, args, { cwd }).return();
  }

  static runScripts(cwd, scriptNames, options) {
    const command = npmCommand(!_.isNil(options) ? options.runScripts || options : options);
